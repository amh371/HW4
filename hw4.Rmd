...
urldata <- "https://raw.githubusercontent.com/amh371/HW4/main/data/politics_twitter.csv"
data <- read_csv(url(urldata)) %>% select(screen_name, text)
...
data$democrats <- str_count(data$party,"Democrat")
data$republicans <- str_count(data$party, "Republican")
sum(data$republicans)
sum(data$democrats)
...
sample <- rbind(dem.sample, rep.sample)
data_split <- initial_split(sample, 1/10)
...
party_recipe <- recipe(party ~ text, data  = train) %>%     step_tokenize() %>%
    step_stopwords() %>% 
    step_stem() %>%
    step_untokenize() %>%
    step_ngram(abc, n=3) %>%
...
lasso <- logistic_reg(penalty = 0.01, mixture = 1) %>%
  ...
  set_engine("glmnet")
wf <- workflow() %>% party_recipe %>% lasso 
  # Add your recipe and model
```
What is the purpose of using a workflow?
Answer: Using a workflow helps ensure unnecessary steps are not being taken as well as automating other tasks to help streamline a process
...
folds <- vfold_cv(train, v = 5, breaks = 20) # Add an argument
...
Answer: This helps ensure that each fold is fully representative of all the strata of the data.
...
Answer: Collect_predictions and collect_metrics help us obtain and format results produced by tuning functions. These metrics can prove the classifier to be accurate.
